...
{
    DataType current_layout_region;
    DataType current_data_region;
    boost::interprocess::named_sharable_mutex &current_regions_mutex;
    DataType old_layout_region;
    DataType old_data_region;
...
Storage::ReturnCode Storage::Run(int max_wait)
{
    ...
    const DataType layout_region = regions_layout.old_layout_region;
    const DataType data_region = regions_layout.old_data_region;
    ...
    SharedDataTimestamp *data_timestamp_ptr = static_cast<SharedDataTimestamp *>(data_type_memory->Ptr());
...
void Storage::LoadLayout(DataLayout *layout_ptr)
{
    ...
    layout_ptr->SetBlockSize<extractor::ProfileProperties>(DataLayout::PROPERTIES, 1);
    ...
    layout_ptr->SetBlockSize<unsigned>(DataLayout::GEOMETRIES_INDEX, number_of_geometries_indices);
    ...
    layout_ptr->SetBlockSize<char>(DataLayout::DATASOURCE_NAME_DATA, m_datasource_name_data.size());
...
void Storage::LoadData(DataLayout *layout_ptr, char *memory_ptr)
{
    ...
    *checksum_ptr = hsgr_header.checksum;
    ...
    extractor::TravelMode *travel_mode_ptr =
        layout_ptr->GetBlockPtr<extractor::TravelMode, true>(memory_ptr, DataLayout::TRAVEL_MODE);
    util::guidance::TurnBearing *pre_turn_bearing_ptr =
        layout_ptr->GetBlockPtr<util::guidance::TurnBearing, true>(memory_ptr,
                                                                   DataLayout::PRE_TURN_BEARING);
    util::guidance::TurnBearing *post_turn_bearing_ptr =
        layout_ptr->GetBlockPtr<util::guidance::TurnBearing, true>(memory_ptr,
                                                                   DataLayout::POST_TURN_BEARING);
    ...
    if (layout_ptr->GetBlockSize(DataLayout::DATASOURCES_LIST) > 0)
    {
        geometry_datasource_input_stream.read(
            ...
            layout_ptr->GetBlockSize(DataLayout::DATASOURCES_LIST));
    }
    ...
    auto datasource_name_offsets_ptr =
        layout_ptr->GetBlockPtr<std::size_t, true>(memory_ptr, DataLayout::DATASOURCE_NAME_OFFSETS);
    ...
    auto datasource_name_lengths_ptr =
        layout_ptr->GetBlockPtr<std::size_t, true>(memory_ptr, DataLayout::DATASOURCE_NAME_LENGTHS);
    ...
    util::Coordinate *coordinates_ptr =
        layout_ptr->GetBlockPtr<util::Coordinate, true>(memory_ptr, DataLayout::COORDINATE_LIST);
    ...
    // store timestamp
    char *timestamp_ptr = layout_ptr->GetBlockPtr<char, true>(memory_ptr, DataLayout::TIMESTAMP);
    io::readTimestamp(timestamp_stream, timestamp_ptr, timestamp_size);

    // store search tree portion of rtree
    char *rtree_ptr = layout_ptr->GetBlockPtr<char, true>(memory_ptr, DataLayout::R_SEARCH_TREE);
    ...
    {
        auto bearing_id_ptr =
            layout_ptr->GetBlockPtr<BearingClassID, true>(memory_ptr, DataLayout::BEARING_CLASSID);
    ...
    if (!bearing_class_table.empty())
    {
        auto bearing_class_ptr =
            layout_ptr->GetBlockPtr<DiscreteBearing, true>(memory_ptr, DataLayout::BEARING_VALUES);
