...
template <class geojson_conversion_policy, LoggingScenario scenario = LoggingScenario::eDefault> class GeojsonLogger
{
  ...
  public:
    ...
    template <typename... Args> static bool Write(Args &&... args)
    {
        ...
        // bookeeping ourselves
        if (!first)
            ofs << ",\n\t\t";

        util::json::render(ofs, json_object);
